...
/**
 * A flexible, sortable list of float primitives.  Borrows much
 * of its functionality from the ArrayFloatList implementation
...
public class SortableFloatList extends ArrayFloatList {
     ...
     */
    public SortableFloatList(int initialCapacity) {
     ...
     */
    public SortableFloatList(FloatCollection that) { 
        this(that.size());
        addAll(that);
    }    
    ...
    /**
     * Sorts the list into ascending numerical order via {@link java.util.Arrays#sort(float[])}
     */
    public void sort() {
        trimToSize();
        Arrays.sort(data);
    ...
    /**
     * Searches the list for the specified key via {@link java.util.Arrays#binarySearch(float[], float)}
     * <p>
     ...
     * via {@link java.util.Arrays#sort(float[], int, int)}
