 ...
 */
public class LongIntervalSet extends LongSet implements IntervalSet {
    private static final int INIT_SIZE = 8;

    protected LongIntervalSetListener cpsListener;
    ...
    protected long[] startVals;
    protected long[] endVals;
    ...
    public LongIntervalSet() {
        startVals = new long[INIT_SIZE];
        endVals = new long[INIT_SIZE];
     ...
     */
    protected LongIntervalSet(LongIntervalSet set) {
        this.longSize = set.longSize;
        this.size = set.size;
        this.firstIntervalIdx = set.firstIntervalIdx;
        this.lastIntervalIdx = set.lastIntervalIdx;
        this.startVals = (long[]) set.startVals.clone();
        this.endVals = (long[]) set.endVals.clone();
     ...
     * @return a long interval set of all the intervals that are free between start and end
     */
     ...
     */
    protected int recordInterval(long start, long end) {
    ...
    private void enlargeArrays() {
        ...
        System.arraycopy(startVals, 0, longTmp, 0, oldSize);
        this.startVals = longTmp;
        
        // Expand endVals
        longTmp = new long[newSize];
        System.arraycopy(endVals, 0, longTmp, 0, oldSize);
        this.endVals = longTmp;
     ...
     */
    public int intervalIdxOf(long val) {
    ...
    public void intersect(LongIntervalSet intersector) {
        ...
        while ((index>=0) && (nextIndex>=0)) {
            long gapStart = intersector.getIntervalEnd(index)+1;
            long gapEnd = intersector.getIntervalStart(nextIndex)-1;
     ...
     */
    private int addWithoutOverlap(long start, long end, int idx, boolean notify) {
     ...
     */
    private int addWithOverlap(long start, long end, int idx, boolean notify) {
        ...
        //                 1    2      3     
        long nextStart = (nextIdx>=0) ? startVals[nextIdx] : 0;
     ...
     */
    private long removeSingleAffectedInterval(long start, long end, long s, long e, int idx, boolean notify) {
        ...
        long removeStart = (s>start) ? s : start;
        long removeEnd = (e<end) ? e : end;
     ...
     * @return the  resulting size change
     */
    private long removeMultiAffectedInterval(long start, long end, long s, long e, int idx, boolean notify) {
        int prevIdx = prevIdxList[idx];
        int nextIdx = nextIdxList[idx];
        long sizeChange = 0;
        ...
        long removeStart = (s < start) ? start : s;
        ...
        // .... .....ooooo ooo ooo  yyyxx
        if (nextIdx >= 0) {
            long nextStart = startVals[nextIdx];
    ...
    public long getNextHigher(long val) {
        ...
        long min = startVals[firstIntervalIdx];
        if (val < min) return min;
        
        // if value is beyond max value, return value passed
        long max = endVals[lastIntervalIdx];
    ...
    public NumberIterator values() {
        return new LongIntervalNumberIterator();
    }
     ...
     */
    public Object clone() {
        return new LongIntervalSet(this);
    }
     ...
     */
    protected void notifyIntervalAddition(long start, long end) {
     ...
     */
    protected void notifyIntervalRemoval(long start, long end) {
        if (cpsListener==null) return;
    ...
    /**
     * Iterator for long intervals
     */
    private class LongIntervalIterator implements IntervalIterator {
        private boolean first;
        private int idx;
        
        public LongIntervalIterator() {
            first = size > 0;
        }
    ...
    private class LongIntervalNumberIterator extends NumberIterator {
        ...
        public LongIntervalNumberIterator() {
     ...
     * @param set   LongIntervalSet to compare this to
...
//    /**
//     * An equals method for comparing if two set objects are equivalent
//     * @param set   LongValIntervalSet to compare this to
//     * @return      true if the objects are determined to be equal
//     */
//    public boolean equals(LongValIntervalSet set) {
//        if ((set.getMin()!=getMin())||
//               (set.getMax()!=getMax())) {
//            return false;
//        }
...
//                return false;
//            }
//            int nextOther = set.getNextIntervalIndex(otherIndex);
//            while ((nextOther>=0) && (set.getIntervalStart(nextOther)-1==set.getIntervalEnd(otherIndex)) 
//                    && (set.getIntervalEnd(nextOther)>set.getIntervalEnd(otherIndex))) {
//                otherIndex = nextOther;
//                nextOther = set.getNextIntervalIndex(nextOther);
//            }
//            
//            if (getIntervalEnd(thisIndex)!= set.getIntervalEnd(otherIndex)) {
//                return false;
