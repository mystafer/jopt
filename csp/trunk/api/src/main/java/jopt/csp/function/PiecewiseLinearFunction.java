 ...
 */
public class PiecewiseLinearFunction implements PiecewiseFunction {
	...
	public PiecewiseLinearFunction(double xmin, double xmax, double val) {
	 ...
	 * for x in [x1, x2), y = y1 + slope * (x-x1)
	 */
	private double calculateY(double x1, double y1, double slope, double x) {
		if (slope != 0)
	...
	public double getY(double x) {
		...
		return calculateY(pointX[idx], pointY[idx], pointSlope[idx], x);
	}
	...
	public void setSlope(double x1, double x2, double y1, double slope) {
		...
		if (startIdx > 0 && pointSlope[startIdx-1] == slope) {
			...
			if (calculateY(prevX, prevY, slope, x1) == y1) {
				x1 = prevX;
				y1 = prevY;
				startIdx--;
			}
		...
		if (endIdx < segmentCount-1 && pointSlope[endIdx+1] == slope) {
			...
			if (calculateY(x1, x2, slope, nextX) == nextY) {
				x2 = nextX;
				endIdx++;
			}
		}
	...
	/**
	 * <p>
	 * Resets function to a piecewise linear set of segments defined by X and Y within the defined interval
	 * [minX, maxX).
	 * </p>
	 * 
	 * y[0] for [minX, x[0])<br>
     * y[i] to y[i+1] for all intermediate points<br>
	 * y[n-1] for [x[n-1], maxX)
	 * 
	...
	public void setPoints(double x[], double y[]) {
		...
		if (x.length != y.length)
			throw new IllegalArgumentException("Number of points in array X must be equal to number of values in array Y");
		...
		// line from y[i] to y[i+1] for all intermediate points
		for (int i=1; i<x.length-1; i++) {
			...
			setSlope(x1, x2, y1, slope);
		}
		
		// y[n-1] for [x[n-1], maxX)
		setY(x[x.length-1], maxX, y[y.length-1]);
	}
	 ...
	 */
	public void setMax(PiecewiseLinearFunction f) {
	...
	private void setLinearMax(int idx, double Xn, double Yn, double Mn, double Xe) {
		...
		if (Ms == Mn) {
			...
			if (Bn > Bs) {
				...
				double Yi = calculateY(Xn, Yn, Mn, Xp);
		...
		// lines intersect
			...
			// lines intersect within the range [Xp, Xe)
				...
				if (Mn > Ms) {
					...
					double Yi = calculateY(Xn, Yn, Mn, Xi);
	 ...
	 * both functions must be the same. 
	 */
	public void setMin(PiecewiseLinearFunction f) {
		if (f.minX != minX || f.maxX != maxX)
			throw new IllegalArgumentException("Functions must have same min and max X intervals");
