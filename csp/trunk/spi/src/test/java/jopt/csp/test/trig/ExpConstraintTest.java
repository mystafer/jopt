...
public class ExpConstraintTest extends TestCase {
    		...
    		for (int i=1; i<=2; i++) {
    			for (int j=1; j<=5; j++) {
    				...
    				for (int k=0; k<100; k++) {
    					...
    					solver.addConstraint(new ExpConstraint(aexpr, zexpr));
    					
    					double minZ = Math.exp(a.getMin());
    					double maxZ = Math.exp(a.getMax());
    	...
    	try {
    		...
    		for (int j=1; j<=5; j++) {
    			for (int k=0; k<100; k++) {
    				...
    				double minA = Math.log(z.getMin());
    				double maxA = Math.log(z.getMax());
    				assertEquals("a min", minA, a.getMin(), 0.0001);
    				assertEquals("a max", maxA, a.getMax(), 0.0001);
    				assertEquals("z min", minZ, z.getMin(), 0.0001);
    				assertEquals("z max", maxZ, z.getMax(), 0.0001);
