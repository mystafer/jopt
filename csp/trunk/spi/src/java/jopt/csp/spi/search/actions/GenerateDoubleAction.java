 ...
 */
public class GenerateDoubleAction extends AbstractSearchNodeAction {
	private CspDoubleVariable vars[];
	 ...
	 */
	public GenerateDoubleAction(CspDoubleVariable vars[], double precision) {
	 ...
	 */
	public GenerateDoubleAction(CspDoubleVariable vars[], double precision, boolean minFirst, VariableSelector varSelector) {
		this.vars = vars;
	...
	public SearchAction performAction() throws PropagationFailureException {
		...
		if(varSelector != null) {
			...
			while(varSelector.hasNext()) {
				CspDoubleVariable var = (CspDoubleVariable) varSelector.next();

				// don't bother instantiating a variable that is already bound
				if (!var.isBound()) {
					instantiateActions.add(new InstantiateDoubleAction(var, precision, minFirst));
				}
			}
		}
		else {
			for (int i=0; i<vars.length; i++) {
				CspDoubleVariable var = vars[i];
