 ...
 */
public class GenericFloatNode extends GenericNumNode  {
    //NOTE: min/max delta changes are tracked in summation arcs rather than in generic nodes
//    ReversibleFloatArray minDeltas;
//    ReversibleFloatArray maxDeltas;
     ...
     */
    public GenericFloatNode(String name, GenericIndex indices[], NumNode nodes[]) {
        super(name, indices, nodes, NumConstants.FLOAT);
//        minDeltas = new ReversibleFloatArray(nodes.length);
//        maxDeltas = new ReversibleFloatArray(nodes.length);
    }
