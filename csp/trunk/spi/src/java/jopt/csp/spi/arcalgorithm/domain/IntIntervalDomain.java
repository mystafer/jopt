 ...
 */
public class IntIntervalDomain extends BaseIntDomain implements IntIntervalSetListener {
    ...
    private ChoicePointMultiIntArray cpValues;
    private ChoicePointMultiIntArray cpDeltaAdded;
    private ChoicePointMultiIntArray cpDeltaRemoved;
    
    /**
     * Constructor for cloning
     */
    private IntIntervalDomain(IntIntervalDomain domain) {
        super();
        ...
        ((IntIntervalSet) values).setListener(this, CALLBACK_VALUES);
        ((IntIntervalSet) delta).setListener(this, CALLBACK_DELTA);
    }
    
    /**
     * Constructor
     */
    public IntIntervalDomain(int min, int max) {
        super();
    ...
    public Object clone() {
        return new IntIntervalDomain(this);
    }
    ...
    public void setChoicePointStack(ChoicePointStack cps) {
        ...
        if (!this.nestedChild) {
	        ...
	        if (cps==null) {
	            ...
	            this.cpValues = cpdata.newMultiIntList(2);
	            this.cpDeltaAdded = cpdata.newMultiIntList(2);
	            this.cpDeltaRemoved = cpdata.newMultiIntList(2);
	        }
        }
    }
    ...
    public final void beforeChoicePointPushEvent() {
        ...
        while (interIter.hasNext()) {
            int offset = cpDeltaAdded.add(0, interIter.nextInt());
            cpDeltaAdded.set(1, offset, interIter.endInt());
        }
        
        interSet = (IntervalSet) deltaRemovedRecord;
        interIter = interSet.intervals();
        while (interIter.hasNext()) {
            int offset = cpDeltaRemoved.add(0, interIter.nextInt());
            cpDeltaRemoved.set(1, offset, interIter.endInt());
        }
    }
