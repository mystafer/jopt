...
import jopt.csp.spi.solver.ChoicePointMultiFloatArray;
 ...
 */
public class FloatIntervalDomain extends BaseFloatDomain implements FloatIntervalSetListener {
    ...
    private ChoicePointMultiFloatArray cpValues;
    private ChoicePointMultiFloatArray cpDeltaAdded;
    private ChoicePointMultiFloatArray cpDeltaRemoved;
    
    /**
     * Constructor for cloning
     */
    private FloatIntervalDomain(FloatIntervalDomain domain) {
        super();
        
        this.nestedChild = domain.nestedChild;
        this.values.addAll(domain.values);
        this.delta.addAll(domain.delta);
        ((FloatIntervalSet) values).setListener(this, CALLBACK_VALUES);
        ((FloatIntervalSet) delta).setListener(this, CALLBACK_DELTA);
    }
    
    /**
     * Constructor
     */
    public FloatIntervalDomain(float min, float max) {
        super();
    ...
    public Object clone() {
        return new FloatIntervalDomain(this);
    }
    ...
    public final void setChoicePointStack(ChoicePointStack cps) {
        ...
        if (!this.nestedChild) {
	        ...
	        if (cps==null) {
	            ...
	            this.cpValues = cpdata.newMultiFloatList(2);
	            this.cpDeltaAdded = cpdata.newMultiFloatList(2);
	            this.cpDeltaRemoved = cpdata.newMultiFloatList(2);
	        }
        }
    }
    ...
    public final void beforeChoicePointPushEvent() {
        ...
        while (interIter.hasNext()) {
            int offset = cpDeltaAdded.add(0, interIter.nextFloat());
            cpDeltaAdded.set(1, offset, interIter.endFloat());
        }
        
        interSet = (IntervalSet) deltaRemovedRecord;
        interIter = interSet.intervals();
        while (interIter.hasNext()) {
            int offset = cpDeltaRemoved.add(0, interIter.nextFloat());
            cpDeltaRemoved.set(1, offset, interIter.endFloat());
        }
    }
