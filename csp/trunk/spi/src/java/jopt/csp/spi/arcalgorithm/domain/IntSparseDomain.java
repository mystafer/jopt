...
import jopt.csp.util.IntSparseSetListener;
 ...
 */
public class IntSparseDomain extends BaseIntDomain implements IntSparseSetListener {
    ...
    private ChoicePointIntArray cpValues;
    private ChoicePointIntArray cpDeltaAdded;
    private ChoicePointIntArray cpDeltaRemoved;
    
    /**
     * Constructor for cloning
     */
    private IntSparseDomain(IntSparseDomain domain) {
        super();
        
        this.nestedChild = domain.nestedChild;
        this.values.addAll(domain.values);
        this.delta.addAll(domain.delta);
        ((IntSparseSet) values).setListener(this, CALLBACK_VALUES);
        ((IntSparseSet) delta).setListener(this, CALLBACK_DELTA);
    }
    
    /**
     * Constructor for cloning
     */
    public IntSparseDomain(IntSparseSet vals) {
        super();
     ...
     */
    public IntSparseDomain(int min, int max) {
        super();
    ...
    public Object clone() {
        return new IntSparseDomain(this);
    }
    ...
    public final void setChoicePointStack(ChoicePointStack cps) {
        ...
        if (!this.nestedChild) {
	        ...
	        if (cps==null) {
	            ...
	            this.cpValues = cpdata.newIntList();
	            this.cpDeltaAdded = cpdata.newIntList();
	            this.cpDeltaRemoved = cpdata.newIntList();
	        }
        }
    }
    ...
    public final void beforeChoicePointPushEvent() {
        ...
        while (numIter.hasNext()) {
        	numIter.next();
            cpDeltaAdded.add(numIter.intValue());
        }
        
        numIter = deltaRemovedRecord.values();
        while (numIter.hasNext()) {
            numIter.next();
            cpDeltaRemoved.add(numIter.intValue());
        }
    }
