...
/**
 * Contains multiple integer arrays that store data for an object that needs to record changes in a
 ...
 */
public class ChoicePointMultiIntArray implements ChoicePointNumArraySet.StackList {
	private ArrayIntList basePointerStack;
    private ArrayIntList sizeStack;
    private int dataStacks[][];
    ...
    /**
     * Creates a new <code>ChoicePointMultiIntArray</code>
     * 
     * @param listCnt   Number of lists to maintain
     */
    public ChoicePointMultiIntArray(int listCnt) {
        ...
        this.dataStacks = new int[listCnt][capacity];
     ...
     */
    public int add(int listIdx, int val) {
        int offset = size;
    	set(listIdx, size, val);
        return offset;
    }
     ...
     */
    public void set(int listIdx, int offset, int val) {
        if (offset < 0)
     ...
     * @param offset    Offset of value in list
     */
    public int get(int listIdx, int offset) {
        if (offset < 0)
            throw new IndexOutOfBoundsException("invalid value offset");
     ...
     * @param offset    Offset of value in list
     */
    public int remove(int listIdx, int offset) {
        if (offset < 0)
            throw new IndexOutOfBoundsException("invalid value offset");
        ...
        // convert to an actual offset in arrays
        int actualOffset = basePointer + offset;
        int dataStack[] = dataStacks[listIdx];
        int val = dataStack[actualOffset]; 
