...
/**
 * Contains multiple long arrays that store data for an object that needs to record changes in a
 ...
 */
public class ChoicePointMultiLongArray implements ChoicePointNumArraySet.StackList {
	private ArrayIntList basePointerStack;
    private ArrayIntList sizeStack;
    private long dataStacks[][];
    ...
    public ChoicePointMultiLongArray(int listCnt) {
        ...
        this.dataStacks = new long[listCnt][capacity];
     ...
     */
    public int add(int listIdx, long val) {
        int offset = size;
     ...
     */
    public void set(int listIdx, int offset, long val) {
        if (offset < 0)
     ...
     * @param offset    Offset of value in list
     */
    public long get(int listIdx, int offset) {
        if (offset < 0)
            throw new IndexOutOfBoundsException("invalid value offset");
     ...
     * @param offset    Offset of value in list
     */
    public long remove(int listIdx, int offset) {
        if (offset < 0)
            throw new IndexOutOfBoundsException("invalid value offset");
        ...
        // convert to an actual offset in arrays
        int actualOffset = basePointer + offset;
        long dataStack[] = dataStacks[listIdx];
