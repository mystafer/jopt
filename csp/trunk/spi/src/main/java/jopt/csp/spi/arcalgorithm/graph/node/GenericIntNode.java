 ...
 */
public class GenericIntNode extends GenericNumNode  {
    //NOTE: min/max delta changes are tracked in summation arcs rather than in generic nodes
//    ReversibleIntArray minDeltas;
//    ReversibleIntArray maxDeltas;
     ...
     */
    public GenericIntNode(String name, GenericIndex indices[], NumNode nodes[]) {
        super(name, indices, nodes, NumConstants.INTEGER);
//        minDeltas = new ReversibleIntArray(nodes.length);
//        maxDeltas = new ReversibleIntArray(nodes.length);
    }
