...
/**
 * Test generic booleans with the xor operation
 *  
 * @author Chris Johnson
 */
public class GenericXorConstraintTest extends TestCase {
    ...
    public GenericXorConstraintTest(java.lang.String testName) {
        super(testName);
    }
    ...
    public void testSettingX1FalseGenericVarConstTrue() {
        try {
            BooleanXorConstraint constraint = new BooleanXorConstraint(xiexpr, y, false, true);
            ...
            assertFalse("y isFalse should be false", y.isFalse());
            //The other variables should are determined
            assertFalse("x2 isTrue should be false", x2.isTrue());
    ...
    public void testGenericConstantTruePropagation() {
        try {
            BooleanXorConstraint constraint = new BooleanXorConstraint(xiexpr, yiconst, ziexpr);
        	store.addConstraint(constraint);
        	store.propagate();
...
//          Now we will propagate
    ...
    public void testSettingX1FalseGenericVarConstFalse() {
        try {
            BooleanXorConstraint constraint = new BooleanXorConstraint(xiexpr, y, false, false);
        	store.addConstraint(constraint);
        	assertFalse("the unbound constraint is not true", constraint.isTrue());
            assertFalse("the unbound constraint is not false", constraint.isFalse());
            //Now we determine one of the variables
